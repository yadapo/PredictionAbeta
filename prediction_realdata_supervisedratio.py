...
if __name__ == "__main__":
    ...
    abeta_insoluble_cortex_df["cortex"] = abeta_insoluble_cortex_df["Abeta40"]
    ...
    abeta_insoluble_hip_df["hippocampus"] = abeta_insoluble_hip_df["Abeta40"]
    ...
    # Pre Visualization ####################
    fig_xdata = plt.figure(figsize=(12, 10))
    ...
    figure_X_PC = plt.figure(figsize=(5, 5))
    ...
    for s_ind in range(dim_s):
        for timing in time_points:
            ...
            if timing == 4:
                ...
                pp_mark = 's'
            pp_format = pp_color + pp_mark
            plt.plot(pc_X_unsupervised[np.logical_and(unsupervised_s == s_ind, unsupervised_t == timing), 0], pc_X_unsupervised[np.logical_and(unsupervised_s == s_ind, unsupervised_t == timing), 1], pp_format)
    for s_ind in range(dim_s):
        for timing in time_points:
            if s_ind == 0:
                pp_color = 'm'
            ...
            if timing == 4:
                ...
                pp_mark = 's'
            pp_format = pp_color + pp_mark
            plt.plot(pc_X_supervised[np.logical_and(supervised_s == s_ind, supervised_t == timing), 0], pc_X_supervised[np.logical_and(supervised_s == s_ind, supervised_t == timing), 1], pp_format)
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    ...
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    # Control the ration of supervised samples
    # # Select 6, 12 samples randomly for 3 times.
    # %%%%%%%%%%%%
    tmp_data_df = data_df.copy()
    tmp_abeta_df = abeta_df.copy()
    tmp_both_data_df = both_data_df.copy()
    homedir_path = os.environ['HOME']
    np.random.seed(0)
    candidate_supervise_sample_num = np.array([6, 6, 6, 9, 9, 18])  # originally the number of supervised samples is 18.
    use_ID_list = []
    nonuse_ID_list = []
    three_time_skf = StratifiedKFold(n_splits=3, random_state=0, shuffle=True)
    for nonuse_inds, use_inds in three_time_skf.split(both_data_df['individualID'], both_data_df['age']):
        use_ID_list.append(both_data_df['individualID'].iloc[use_inds].values)
        nonuse_ID_list.append(both_data_df['individualID'].iloc[nonuse_inds].values)
    two_time_skf = StratifiedKFold(n_splits=2, random_state=0, shuffle=True)
    for nonuse_inds, use_inds in two_time_skf.split(both_data_df['individualID'], both_data_df['age']):
        use_ID_list.append(both_data_df['individualID'].iloc[use_inds].values)
        nonuse_ID_list.append(both_data_df['individualID'].iloc[nonuse_inds].values)
    use_ID_list.append(both_data_df['individualID'].values)
    nonuse_ID_list.append([])
    mse_trial_results = np.zeros([candidate_supervise_sample_num.shape[0]])

    for trial_ind, supervise_sample_num in enumerate(candidate_supervise_sample_num):
        data_df = tmp_data_df.copy()
        abeta_df = tmp_abeta_df.copy()
        both_data_df = tmp_both_data_df.copy()

        supervise_sample_IDarray = use_ID_list[trial_ind]
        non_selected_sample_IDarray = nonuse_ID_list[trial_ind]
        for non_selected_sample_ID in non_selected_sample_IDarray:
            abeta_df['individualID'][abeta_df['individualID']==non_selected_sample_ID] = non_selected_sample_ID + 10000
            both_data_df = both_data_df[both_data_df['individualID'] != non_selected_sample_ID]
        print(abeta_df['individualID'].iloc[:30])
    ...
    #%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for train_index, test_index in loo.split(both_data_df):
            ...
            mcmc.run(rng_key, Z_obs=Z, t_obs=t, s_obs=s, mu_beta_prior=beta, mu_q_prior=q, mu_alpha_prior=alpha)
            mcmc.print_summary()
            # az.plot_trace(mcmc, var_names=["beta[1,0]", "q[1,0]"])
            ...
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_original_abeta_timecourse" + str(testset_ind) + ".eps")
            fig_abeta.show()
            ...
            for s_ind in range(2):
                ...
                for samp_ind in range(100):
                    ...
                    q_samp = truncnorm.rvs((0.0 - mu_q_samp[samp_ind]) / sigma_q_samp[samp_ind],
                                           (np.inf - mu_q_samp[samp_ind]) / sigma_q_samp[samp_ind],
                                           loc=mu_q_samp[samp_ind], scale=sigma_q_samp[samp_ind], size=1)
                    if s_ind == 0:
                        plt_color = 'r'
                    else:
                        plt_color = 'k'
            ...
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_abeta_estimate_timecourse" + str(testset_ind) + ".eps")
        ...
        ##########################
            ...
            mcmc.run(rng_key, X_obs=unsupervised_X, t_obs=unsupervised_t, s_obs=unsupervised_s, supervised_X_obs=supervised_X, supervised_Z_obs=supervised_Z, supervised_t_obs=supervised_t, supervised_s_obs=supervised_s, mu_beta_prior=mu_beta_prior, mu_q_prior=mu_q_prior, mu_alpha_prior=mu_alpha_prior, mu_beta_sigma=mu_beta_sigma_prior, mu_q_sigma=mu_q_sigma_prior, mu_alpha_sigma=mu_alpha_sigma_prior, prec_beta_shape=prec_beta_shape_prior, prec_beta_scale=prec_beta_scale_prior, prec_q_shape=prec_q_shape_prior, prec_q_scale=prec_q_scale_prior, prec_alpha_shape=prec_alpha_shape_prior, prec_alpha_scale=prec_alpha_scale_prior, grad_Z_correction_factor=grad_Z_correction_factor)
            mcmc.print_summary()
            #az.plot_trace(mcmc, var_names=["beta[1,0]", "q[1,0]"])
            posterior_samples = mcmc.get_samples()
            ...
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_trueZ_vs_latentZ"+str(testset_ind)+".png")
            ...
            plt.ylim(-0.05,1.05)
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_latentZ_wt_vs_tg"+str(testset_ind)+".eps")
            ...
            plt.xlabel('Age [m.o.]')
            plt.ylabel('Latent abeta')
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_latentZ_timecourse"+str(testset_ind)+".eps")
            ...
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_gradZ_timecourse" + str(testset_ind) + ".eps")
            ...
            plt.plot(Z_points, Z_logprob[0], 'ro', markersize=2)
            plt.xlabel("Z")
            plt.ylabel("log(Density)")
            fig_test_Z_estimate.tight_layout()
            plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_rawX_Z_estimate_testset"+str(testset_ind)+".eps")
            fig_test_Z_estimate.show()

            dill.dump_session("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_session_after_mcmc4real_testset"+str(testset_ind)+".pkl")
            testset_ind += 1
        ...
        mse_trial_results[trial_ind] = mse_z_pred
        ...
        plt.savefig("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_truetestZ_vs_predictedZ_MAP.eps")
        fig_truetestZ_vs_predictedZ.show()

        dill.dump_session("trial"+str(trial_ind)+"_supervised_samp"+str(supervise_sample_num)+"_session_after_mcmc4real_final.pkl")

        #$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$

    print(mse_trial_results)
    fig_supervised_ratio_mse = plt.figure(figsize=(5, 5))
    plt.rcParams['font.size'] = '16'
    mse_mean_half = np.mean(mse_trial_results[candidate_supervise_sample_num == 9])
    mse_mean_third = np.mean(mse_trial_results[candidate_supervise_sample_num == 6])
    plt.plot(18, mse_trial_results[candidate_supervise_sample_num == 18], 'ro')
    plt.plot(9, mse_mean_half, 'ro')
    plt.plot(6, mse_mean_third, 'ro')
    plt.plot([6,9,18],[mse_mean_third, mse_mean_half, mse_trial_results[candidate_supervise_sample_num == 18]], 'r-', linewidth=2.0)
    plt.scatter(candidate_supervise_sample_num, mse_trial_results, facecolor='None', edgecolor='k')
    plt.xlim(4, 20)
    plt.xticks([6,9,18])
    plt.ylim(0, 0.1)
    plt.xlabel('The number of supervised samples')
    plt.ylabel('MSE')
    plt.savefig("supervised_ratio_mse.eps")
    fig_supervised_ratio_mse.show()

    dill.dump_session("session_final.pkl")